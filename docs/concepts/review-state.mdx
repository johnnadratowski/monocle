...
- **Binary files** — content hashes detect changes, but monocle shows "binary file changed" instead of a diff. The test is how *dense* control bytes are, not whether any are present: a source file carrying a stray NUL or escape still shows its diff, with the byte drawn as a `␀`-style glyph so you can see it. That byte is often the whole change.
