...
| `markdown_viewer` | string | `""` | Rendered-markdown viewer launched by `Ctrl+p` for the current artifact or `.md` file. May include flags and quoted arguments (e.g. `open -a "Google Chrome"` on macOS, or `glow -p`). Empty falls back to [`glow -p`](https://github.com/charmbracelet/glow) (its scrollable pager). |
