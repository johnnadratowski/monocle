...
When registered in **MCP tools mode**, agents call these operations as MCP tools (`review_status`, `get_feedback`, `send_artifact`, `add_files`, `remove_files`) instead of CLI commands. The behavior is identical — the MCP server connects to the same Unix socket internally.
