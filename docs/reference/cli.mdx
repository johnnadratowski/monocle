...
Start the built-in MCP server over stdio. This exposes monocle's review operations (`review_status`, `get_feedback`, `send_artifact`, `add_files`, `remove_files`) as MCP tools that any stdio-compatible agent can call.
