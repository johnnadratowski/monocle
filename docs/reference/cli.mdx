...
Start the built-in MCP server over stdio. This exposes monocle's review operations (`review_status`, `get_feedback`, `send_artifact`, `add_files`, `remove_files`, `set_file_groups`) as MCP tools that any stdio-compatible agent can call.
