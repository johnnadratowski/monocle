...
The diff pane's top border also shows how long ago the open item last changed, next to its name — for an artifact, when the agent last sent it; for a file, its modification time. Ages read as `now`, `5m`, `3h`, `2d`, `3w`. On a narrow pane the age is dropped first, then the mode badge, so the file name always stays visible.
